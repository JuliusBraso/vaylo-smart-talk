...
const PHOTO_OCR_EPISTEMIC = [
  ...
  "Photo OCR procedural dates (Phase 7.8C–D): OCR can insert or misplace calendar tokens; do NOT treat DD.MM.YYYY as Zahlungs-/Einspruchsfrist unless that exact token appears in the transcript window next to deadline/payment/procedural cues (the same proximity rule the backend enforces for JSON arrays and for prose fields summary/meaning/stabilizers). Prefer relative Frist wording from the text over a lone date.",
...
const PHOTO_OCR_MULTI_PAGE =
  "Multi-page OCR package: Markers === SMART_TALK_PAGE N === are strictly ordered (1 first). Scan every page equally—later sheets often hold deadlines, appeals, annexes, calculation tables, or legal warnings; do not overweight page 1. Treat annexes as part of one administrative package for classification and advice. If pages contradict amounts, dates, or obligations, reconcile conservatively (state uncertainty; prefer lower confidenceLevel where conflict or weak OCR); never invent resolution.";
...
/**
 * Phase 7.9A — Slovak semantic localization: natural assistant wording while preserving 7.8 grounding.
 * Not machine translation; do not weaken obligations, deadlines, or consequences.
 */
const SEMANTIC_LOCALIZATION_RULES = [
  "Semantic legal localization — Slovak locale only (Phase 7.9A): Write like a calm Slovak assistant helping someone navigate German bureaucracy: readable, human, non-alarmist—not stiff literal German-to-Slovak, not a lawyer-style memo, not oversimplified children's explanations.",
  "Preserve completely: factual grounding, procedural precision, hierarchy (mandatory duties vs optional rights vs purely informational vs conditional/possible consequences), and epistemic hedging from the letter. Do not invent deadlines, rights, penalties, or Slovak legal substitutes for German procedures.",
  "Translate German procedural concepts by meaning (examples are guidance, pick what fits the document): Einspruch → odvolanie / daňové odvolanie when Finanzamt tax context is clear; Bekanntgabe (official notification of a decision) → doručenie rozhodnutia; Rechtsbehelfsbelehrung → poučenie o odvolaní; Säumniszuschlag → poplatok za omeškanie; Vollstreckung → vymáhanie; Mahnung → upomienka; Bescheid → rozhodnutie úradu or daňové rozhodnutie when clearly tax-related; vorläufig → predbežné / zatiaľ nefinálne when the source hedges.",
  "Avoid dumping untranslated German legal jargon into Slovak prose unless it adds clarity; avoid unnatural calques and bureaucratic Slovak that mimics German syntax.",
...
const JSON_KEYS_TEXT = [
  ...
  'summary (string): short plain-language overview; Phase 7.8D — never invent DD.MM.YYYY / YYYY-MM-DD unless that exact substring appears cue-grounded in the source like deadlines[]. If timing is relative-only, stay relative.',
  'meaning (string): procedural explanation: separate what is explicitly stated from timing that depends on Zugang/Bekanntgabe (Phase 7.9A Slovak: prefer doručenie rozhodnutia where Bekanntgabe refers to notifying the decision); avoid sounding like legal advice; never state a computed appeal or payment cutoff as fact; never cite DD.MM.YYYY unless that token appears in the source glued to procedural/payment context (same 7.8C cue proximity as backend). Prefer Slovak relative wording tied to the printed German rule (e.g. one month from doručenie when innerhalb eines Monats nach Bekanntgabe).',
  "nextSteps (string[]): practical steps; NEVER include DD.MM.YYYY or YYYY-MM-DD unless that exact substring exists in the source text near deadline/payment/appeal cues (not a coincidental date elsewhere); for relative Frist/Einspruch windows without a printed end date, direct the user to the stated period only (no calculated day).",
  'warnings (string[]): 1–2 short calm items (material risks, deadlines, procedures); at most one may be a brief stabilizing clarification only if the document explicitly supports it; prioritize contextual risks over generic hygiene; empty array only when genuinely none apply. Never cite a concrete calendar deadline unless the source ties that date to obligation/procedure (see Phase 7.8C); use conditional wording (môže / may) only when supported.',
  'stabilizers (string[]): 0–2 very short items: protective or stabilizing facts explicitly stated in the input (e.g. no final decision yet; no enforcement active yet; lawful residence during pending review; insurance not cancelled—only conditional limitation risk; benefits or payments continue during review; documents received but processing not finished). [] when none. Do not invent reassurance; never embed fabricated calendar deadlines—same 7.8D cue rule as summary/meaning. Prefer stabilizers for these facts; keep warnings primarily for risks; minimize repeating the same fact in warnings and stabilizers.',
  ...
  'deadlines (string[]): literal-only — relative periods as printed (innerhalb eines Monats …) need no calendar day. For DD.MM.YYYY / ISO dates, only when the source ties that date to a payment/procedural rule (same line/paragraph cluster as Frist, zahlbar, spätestens, Einspruch, Bekanntgabe, etc.); ignore stray dates in unrelated sections. NEVER include a calendar date computed or inferred from another date.',
...
export function buildSmartTalkMessages(params: {
  ...
  const localeLine =
    params.locale === "sk"
      ? "Write summary, meaning, nextSteps, warnings, stabilizers, and the short strings inside deadlines / rights / obligations / consequences arrays in Slovak unless quoting German source text verbatim. Keep proceduralState and legalSeverity as the English enum tokens required by JSON."
      : params.locale === "de"
  ...
  if (params.inputType === "text") {
    const system = [
      ...
      ...(params.source === "photo_ocr" && params.text.includes("=== SMART_TALK_PAGE 2 ===")
        ? [PHOTO_OCR_MULTI_PAGE]
        : []),
