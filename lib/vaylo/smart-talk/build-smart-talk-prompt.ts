...
const PROCEDURAL_REASONING_RULES =
  ...
  "nextSteps[] / warnings[]: do NOT include calendar dates (DD.MM.YYYY or YYYY-MM-DD) unless that exact date string appears in the source text next to deadline/payment/procedural cues (7.8C); without cue proximity, use relative or generic guidance (e.g. lehota jeden mesiac od oznámenia). For Finanzamt/Bescheid with Einspruch but no printed end date, use generic guidance—never 'do X by 31.01.2026' unless that date is printed and cue-grounded. Use 'môže vzniknúť' only when supported.\n" +
...
const JSON_KEYS_TEXT = [
  ...
  'meaning (string): procedural explanation consistent with Poučenie o právnych prostriedkoch: separate what is explicitly stated from what depends on Zugang/Bekanntgabe; avoid sounding like legal advice; never state a computed appeal or payment cutoff as fact; never cite DD.MM.YYYY unless that token appears in the source glued to procedural/payment context (same 7.8C cue proximity as backend). Prefer “lehota jeden mesiac od oznámenia” when only relative Frist appears.',
