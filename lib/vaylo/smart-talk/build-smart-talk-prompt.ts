...
const PHOTO_OCR_EPISTEMIC = [
  ...
  "Photo OCR procedural dates (Phase 7.8C): OCR can insert or misplace calendar tokens; do NOT treat DD.MM.YYYY as Zahlungs-/Einspruchsfrist unless that date appears in the transcript window next to deadline/payment/procedural cues (the same proximity rule the backend enforces for JSON arrays). Prefer relative Frist wording from the text over a lone date.",
...
const PROCEDURAL_REASONING_RULES =
  ...
  "OCR procedural grounding (Phase 7.8C): Transcripts may misread digits or contain dates that belong to Kontoauszug/Überschrift rather than Pflicht—they are frequent false anchors. Never copy a stray DD.MM.YYYY into deadlines/nextSteps/warnings/obligations/consequences as authoritative unless the same date appears close to deadline/payment/appeal wording (e.g. Frist, innerhalb … Bekanntgabe, zahlbar, spätestens, Einspruch, Mahnung).\n" +
