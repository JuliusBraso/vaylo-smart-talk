-- CB-0E: add eu_health_insurance_coordination to the EU process-group
-- allowlist and to the EU jurisdiction writer. Additive only.
-- Does not modify 052 or 053. Does not add SK/CZ/PL/HU national ingestion.
-- Does not alter connector lifecycle. No production role grants.
...
begin
  ...
  if v_pack_id is distinct from 'eu_jurisdiction_anchor'
     ...
     and v_pack_id is distinct from 'eu_health_insurance_coordination' then
  ...
  if v_pack_id in ('eu_applicable_legislation', 'eu_health_insurance_coordination')
...
comment on function public.knowledge_ingest_curated_eu_jurisdiction_anchor(jsonb) is
  'CB-0B/0C/0E EU source-jurisdiction writer. Accepts eu_jurisdiction_anchor, eu_applicable_legislation and eu_health_insurance_coordination. Rejects SK/CZ/PL/HU national ingestion. No production role grant.';
