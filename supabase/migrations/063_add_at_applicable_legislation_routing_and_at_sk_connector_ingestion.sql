-- AT-SK-0D: additive AT applicable-legislation routing process group,
-- AT-SK applicable-legislation connector support, and server-only ingest
-- wrappers. Does not activate AT-SK, public runtime, or other domain
-- connectors. Does not modify 062. Do not deploy.
...
begin
  ...
  from pg_constraint
  where conrelid = 'public.knowledge_cross_border_connectors'::regclass
    and contype = 'c'
    and pg_get_constraintdef(oid) ilike '%origin_market%';
  if cname is not null then
    execute format('alter table public.knowledge_cross_border_connectors drop constraint %I', cname);
  end if;
end $$;

alter table public.knowledge_cross_border_connectors
  add constraint knowledge_cross_border_connectors_origin_market_check
    check (origin_market in ('DE', 'AT'));
...
begin
  ...
  elsif p_ref->>'sourceJurisdiction' = 'AT' then
...
begin
  ...
  if p_ref->>'sourceJurisdiction' = 'DE' then
    if p_ref->>'trustDomain' is distinct from 'de' then
      raise exception 'CONNECTOR_WRONG_JURISDICTION';
    end if;
  elsif p_ref->>'sourceJurisdiction' = 'AT' then
    if p_ref->>'trustDomain' is distinct from 'at' then
      raise exception 'CONNECTOR_WRONG_JURISDICTION';
    end if;
  else
    raise exception 'CONNECTOR_WRONG_JURISDICTION';
...
create or replace function public.knowledge_ingest_curated_at_applicable_legislation_routing_pack(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if v_pack_id is distinct from 'at_applicable_legislation_routing' then
    raise exception 'AT_AL_ROUTING_PACK_IDENTITY_INVALID';
  end if;
  if p_payload->>'countryCode' is distinct from 'AT' then
    raise exception 'AT_AL_ROUTING_COUNTRY_NOT_AUTHORIZED';
  end if;
  if p_payload#>>'{trustDomain,code}' is distinct from 'at' then
    raise exception 'AT_AL_ROUTING_TRUST_DOMAIN_INVALID';
  end if;
  if exists (
    ...
    raise exception 'AT_AL_ROUTING_JURISDICTION_INVALID';
  end if;
  if p_payload ? 'userLocale' or p_payload ? 'locale' or p_payload ? 'outputLocale'
     or p_payload ? 'uiLanguage' then
    raise exception 'AT_AL_ROUTING_LOCALE_ACTIVATION_FORBIDDEN';
  end if;
...
create or replace function public.knowledge_ingest_curated_at_sk_applicable_legislation_connector_pack(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if p_payload->>'schemaVersion' <> '1'
     or p_payload->>'packId' is distinct from 'at_sk_applicable_legislation'
  ...
  v_actor := knowledge_factory_internal.stable_knowledge_factory_id(
    ...
    'AT:SK:applicable-legislation-posting-a1'
  );
  ...
  for v_process_row in select value from jsonb_array_elements(v_corridor) loop
    ...
    insert into public.knowledge_processes(
      ...
      v_corridor_process, 'at_sk_applicable_legislation_connector',
...
revoke all on function public.knowledge_ingest_curated_at_applicable_legislation_routing_pack(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function public.knowledge_ingest_curated_at_sk_applicable_legislation_connector_pack(jsonb)
  from public, anon, authenticated, service_role;
...
comment on function public.knowledge_ingest_curated_at_applicable_legislation_routing_pack(jsonb) is
  'AT-SK-0D server-only Austrian applicable-legislation routing ingest. Revoke-by-default. Not deployed.';
comment on function public.knowledge_ingest_curated_at_sk_applicable_legislation_connector_pack(jsonb) is
  'AT-SK-0D server-only AT-SK applicable-legislation connector ingest. Prepared only. Revoke-by-default. Not deployed.';
