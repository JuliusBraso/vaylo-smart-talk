-- CB-0C: expand the CB-0B EU jurisdiction writer so one shared applicable-
-- legislation / posting / multi-state / PD A1 core can be stored once.
-- Additive only. Does not modify 051. Does not add a Knowledge Factory domain.
-- Does not add SK/CZ/PL/HU national ingestion. No production role grants.
...
begin
  ...
  if v_pack_id is distinct from 'eu_jurisdiction_anchor'
     and v_pack_id is distinct from 'eu_applicable_legislation' then
  ...
  if v_pack_id = 'eu_applicable_legislation'
  ...
  for v_row in select value from jsonb_array_elements(v_processes) loop
    insert into public.knowledge_processes(
      ...
      (v_row->>'id')::uuid, 'eu_applicable_legislation', v_row->>'title',
      (v_row->>'jurisdictionId')::uuid, nullif(v_row->>'territorialScopeId', '')::uuid,
...
comment on function public.knowledge_ingest_curated_eu_jurisdiction_anchor(jsonb) is
  'CB-0B/0C EU source-jurisdiction writer. Accepts eu_jurisdiction_anchor and eu_applicable_legislation. Rejects SK/CZ/PL/HU national ingestion. No production role grant.';
