-- CB-0F: allow SK health adapter, DE health routing, and DE↔SK health
-- connector process groups. Additive only. Does not modify 053 or 054.
...
begin
  ...
  if v_pack_id is distinct from 'de_applicable_legislation_routing'
     and v_pack_id is distinct from 'de_health_insurance_coordination_routing' then
...
comment on function public.knowledge_ingest_curated_foreign_national_adapter_pack(jsonb) is
  'CB-0D/0F SK-only foreign-national adapter writer. Applicable-legislation and health-coordination packIds. Rejects CZ/PL/HU. No production role grant.';
comment on function public.knowledge_ingest_curated_de_corridor_routing_pack(jsonb) is
  'CB-0D/0F German corridor routing writer. Applicable-legislation and health-coordination packIds. No production role grant.';
comment on function public.knowledge_ingest_curated_cross_border_connector_pack(jsonb) is
  'CB-0B/0D/0F connector writer. Planned or prepared only. Health corridor processes use the health process group. Never active. No production role grant.';
