-- AT-SK-0C: additive Austrian national foundation trust, jurisdiction level,
-- process group, and server-only ingest wrapper. Does not activate AT-SK,
-- public runtime, or any Austrian domain pack. Do not deploy.
...
alter table public.knowledge_trust_domains
  add constraint knowledge_trust_domains_code_check
    check (code in ('eu', 'de', 'sk', 'cz', 'pl', 'hu', 'bilateral_tax_treaty', 'at'));
...
alter table public.knowledge_jurisdictions
  add constraint knowledge_jurisdictions_jurisdiction_level_check
    check (jurisdiction_level in (
      ...
      'cross_border_multi_jurisdiction', 'unresolved', 'foreign_national', 'at_national'
    ));
...
create or replace function public.knowledge_ingest_curated_at_national_foundation_pack(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if v_pack_id is distinct from 'at_national_foundation' then
    raise exception 'AT_FOUNDATION_PACK_IDENTITY_INVALID';
  end if;
  if p_payload->>'countryCode' is distinct from 'AT' then
    raise exception 'AT_FOUNDATION_COUNTRY_NOT_AUTHORIZED';
  end if;
  if p_payload#>>'{trustDomain,code}' is distinct from 'at' then
    raise exception 'AT_FOUNDATION_TRUST_DOMAIN_INVALID';
  end if;
  if exists (
    ...
    raise exception 'AT_FOUNDATION_JURISDICTION_INVALID';
  end if;
  if p_payload ? 'userLocale' or p_payload ? 'locale' or p_payload ? 'outputLocale'
     or p_payload ? 'uiLanguage' then
    raise exception 'AT_FOUNDATION_LOCALE_ACTIVATION_FORBIDDEN';
  end if;
...
revoke all on function public.knowledge_ingest_curated_at_national_foundation_pack(jsonb)
  from public, anon, authenticated, service_role;

comment on function public.knowledge_ingest_curated_at_national_foundation_pack(jsonb) is
  'AT-SK-0C server-only Austrian national foundation ingest. Revoke-by-default. Not deployed. Not a public runtime surface.';
