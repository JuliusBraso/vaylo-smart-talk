-- CB-0J: allow SK unemployment adapter, DE unemployment routing, and
-- DE↔SK unemployment connector process groups. Additive only.
-- Does not modify 052–058. Reuses existing foreign_national SK jurisdiction
-- and connector tables. Does not open CZ/PL/HU national ingestion.
-- Does not activate the corridor. No production role grants.
...
begin
  ...
  if p_payload->>'countryCode' is distinct from 'SK'
     or (
       v_pack_id is distinct from 'sk_applicable_legislation_adapter'
       and v_pack_id is distinct from 'sk_health_insurance_coordination_adapter'
       and v_pack_id is distinct from 'sk_family_benefits_adapter'
       and v_pack_id is distinct from 'sk_unemployment_coordination_adapter'
     ) then
...
begin
  v_pack_id := p_payload->>'packId';
  if v_pack_id is distinct from 'de_applicable_legislation_routing'
     and v_pack_id is distinct from 'de_health_insurance_coordination_routing'
     and v_pack_id is distinct from 'de_family_benefits_coordination_routing'
     and v_pack_id is distinct from 'de_unemployment_coordination_routing' then
...
begin
  ...
  v_process_group := case p_payload->>'packId'
    when 'de_sk_unemployment_coordination' then 'de_sk_unemployment_coordination_connector'
    when 'de_sk_family_benefits_coordination' then 'de_sk_family_benefits_coordination_connector'
    when 'de_sk_health_insurance_coordination' then 'de_sk_health_insurance_coordination_connector'
    else 'de_sk_applicable_legislation_connector'
  end;
  ...
  for v_process_row in select value from jsonb_array_elements(v_corridor) loop
    ...
    insert into public.knowledge_processes(
      ...
      v_corridor_process, v_process_group,
...
comment on function public.knowledge_ingest_curated_foreign_national_adapter_pack(jsonb) is
  'CB-0D/0F/0H/0J SK-only foreign-national adapter writer. Applicable-legislation, health, family and unemployment packIds. Rejects CZ/PL/HU. No production role grant.';
comment on function public.knowledge_ingest_curated_de_corridor_routing_pack(jsonb) is
  'CB-0D/0F/0H/0J German corridor routing writer. Applicable-legislation, health, family and unemployment packIds. No production role grant.';
comment on function public.knowledge_ingest_curated_cross_border_connector_pack(jsonb) is
  'CB-0B/0D/0F/0H/0J connector writer. Planned or prepared only. Unemployment corridor processes use the unemployment process group. Never active. No production role grant.';
