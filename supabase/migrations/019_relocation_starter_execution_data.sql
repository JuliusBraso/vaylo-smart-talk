-- Phase 5.1b: Relocation Starter execution data (idempotent, connected).
-- Notes:
-- - Uses stable IDs for topics/steps/doc types.
-- - Steps use ON CONFLICT (id) DO UPDATE to allow safe evolution.
-- - Dependencies/links use ON CONFLICT DO NOTHING (edges are append-only).
...
-- ---------------------------------------------------------------------------
-- SELECT * FROM knowledge_steps WHERE topic_id = 'relocation_basics';
-- SELECT * FROM knowledge_step_dependencies WHERE step_id IN ('anmeldung', 'tax_id');
