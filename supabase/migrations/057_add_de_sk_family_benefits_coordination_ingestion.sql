-- CB-0H: allow SK family adapter, DE family routing, and DE↔SK family
-- connector process groups. Additive only. Does not modify 052–056.
-- Reuses existing foreign_national SK jurisdiction and connector tables.
-- Does not open CZ/PL/HU national ingestion. No production role grants.
...
begin
  ...
  if v_pack_id is distinct from 'de_applicable_legislation_routing'
     ...
     and v_pack_id is distinct from 'de_family_benefits_coordination_routing' then
...
comment on function public.knowledge_ingest_curated_foreign_national_adapter_pack(jsonb) is
  'CB-0D/0F/0H SK-only foreign-national adapter writer. Applicable-legislation, health-coordination and family-benefits packIds. Rejects CZ/PL/HU. No production role grant.';
comment on function public.knowledge_ingest_curated_de_corridor_routing_pack(jsonb) is
  'CB-0D/0F/0H German corridor routing writer. Applicable-legislation, health-coordination and family-benefits packIds. No production role grant.';
comment on function public.knowledge_ingest_curated_cross_border_connector_pack(jsonb) is
  'CB-0B/0D/0F/0H connector writer. Planned or prepared only. Family corridor processes use the family process group. Never active. No production role grant.';
