-- CB-0G: add eu_family_benefits_coordination to the EU process-group
-- allowlist and to the EU jurisdiction writer. Additive only.
-- Does not modify 052, 054 or 055. Does not add SK/CZ/PL/HU national ingestion.
-- Does not create a DE↔SK family connector. No production role grants.
...
begin
  ...
  if v_pack_id is distinct from 'eu_jurisdiction_anchor'
     ...
     and v_pack_id is distinct from 'eu_family_benefits_coordination' then
...
comment on function public.knowledge_ingest_curated_eu_jurisdiction_anchor(jsonb) is
  'CB-0B/0C/0E/0G EU source-jurisdiction writer. Accepts eu_jurisdiction_anchor, eu_applicable_legislation, eu_health_insurance_coordination and eu_family_benefits_coordination. Rejects SK/CZ/PL/HU national ingestion. No production role grant.';
