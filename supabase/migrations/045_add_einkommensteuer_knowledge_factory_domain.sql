-- Minimum Knowledge Factory taxonomy extension for Einkommensteuer /
-- Steuererklärung federal lifecycle. Additive only: extends the
...
begin
  ...
  if v_definition not like '%einkommensteuer_steuererklaerung%' then
    v_definition := replace(
      v_definition,
      $lit$'arbeitslosengeld'$lit$,
      $lit$'arbeitslosengeld','einkommensteuer_steuererklaerung'$lit$
    );
