-- AT-SK-0G: additive AT unemployment coordination routing process group,
-- AT-SK unemployment connector support, and server-only ingest wrappers.
-- Does not activate AT-SK, public runtime, or other domain connectors.
-- Does not modify 065. Does not drop origin_market or resolver functions.
-- Do not deploy.
...
create or replace function public.knowledge_ingest_curated_at_unemployment_coordination_routing_pack(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if v_pack_id is distinct from 'at_unemployment_coordination_routing' then
    raise exception 'AT_UE_ROUTING_PACK_IDENTITY_INVALID';
  end if;
  if p_payload->>'countryCode' is distinct from 'AT' then
    raise exception 'AT_UE_ROUTING_COUNTRY_NOT_AUTHORIZED';
  end if;
  if p_payload#>>'{trustDomain,code}' is distinct from 'at' then
    raise exception 'AT_UE_ROUTING_TRUST_DOMAIN_INVALID';
  end if;
  if exists (
    ...
    raise exception 'AT_UE_ROUTING_JURISDICTION_INVALID';
  end if;
  if p_payload ? 'userLocale' or p_payload ? 'locale' or p_payload ? 'outputLocale'
     or p_payload ? 'uiLanguage' then
    raise exception 'AT_UE_ROUTING_LOCALE_ACTIVATION_FORBIDDEN';
  end if;
...
create or replace function public.knowledge_ingest_curated_at_sk_unemployment_coordination_connector_pack(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if p_payload->>'schemaVersion' <> '1'
     or p_payload->>'packId' is distinct from 'at_sk_unemployment_coordination'
  ...
  v_actor := knowledge_factory_internal.stable_knowledge_factory_id(
    ...
    'AT:SK:unemployment-coordination-arbeitslosengeld-davka'
  );
  ...
  for v_process_row in select value from jsonb_array_elements(v_corridor) loop
    ...
    insert into public.knowledge_processes(
      ...
      v_corridor_process, 'at_sk_unemployment_coordination_connector',
...
revoke all on function public.knowledge_ingest_curated_at_unemployment_coordination_routing_pack(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function public.knowledge_ingest_curated_at_sk_unemployment_coordination_connector_pack(jsonb)
  from public, anon, authenticated, service_role;
...
comment on function public.knowledge_ingest_curated_at_unemployment_coordination_routing_pack(jsonb) is
  'AT-SK-0G server-only Austrian unemployment routing ingest. Revoke-by-default. Not deployed.';
comment on function public.knowledge_ingest_curated_at_sk_unemployment_coordination_connector_pack(jsonb) is
  'AT-SK-0G server-only AT-SK unemployment coordination connector ingest. Prepared only. Revoke-by-default. Not deployed.';
