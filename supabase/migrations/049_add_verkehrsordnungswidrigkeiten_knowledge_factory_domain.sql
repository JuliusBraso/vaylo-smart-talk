-- Minimum Knowledge Factory taxonomy extension for road-traffic
-- administrative-offence / Bußgeldverfahren lifecycle. Additive only:
-- extends the process-group check and the saved 041 domain whitelist.
...
begin
  ...
  if v_definition not like '%verkehrsordnungswidrigkeiten_bussgeldverfahren%' then
    v_definition := replace(
      v_definition,
      $lit$'banking_zahlungsverkehr'$lit$,
      $lit$'banking_zahlungsverkehr','verkehrsordnungswidrigkeiten_bussgeldverfahren'$lit$
    );
