-- AT-SK-0H: additive AT cross-border Gewerbe/service routing process group,
-- AT-SK Gewerbe/service connector support, and server-only ingest wrappers.
-- Does not activate AT-SK, public runtime, or other domain connectors.
-- Does not modify 066. Does not drop origin_market or resolver functions.
-- Do not deploy.
...
create or replace function public.knowledge_ingest_curated_at_cross_border_gewerbe_service_routing_pack(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if v_pack_id is distinct from 'at_cross_border_gewerbe_service_routing' then
    raise exception 'AT_GEWERBE_ROUTING_PACK_IDENTITY_INVALID';
  end if;
  if p_payload->>'countryCode' is distinct from 'AT' then
    raise exception 'AT_GEWERBE_ROUTING_COUNTRY_NOT_AUTHORIZED';
  end if;
  if p_payload#>>'{trustDomain,code}' is distinct from 'at' then
    raise exception 'AT_GEWERBE_ROUTING_TRUST_DOMAIN_INVALID';
  end if;
  if exists (
    ...
    raise exception 'AT_GEWERBE_ROUTING_JURISDICTION_INVALID';
  end if;
  if p_payload ? 'userLocale' or p_payload ? 'locale' or p_payload ? 'outputLocale'
     or p_payload ? 'uiLanguage' then
    raise exception 'AT_GEWERBE_ROUTING_LOCALE_ACTIVATION_FORBIDDEN';
  end if;
...
create or replace function public.knowledge_ingest_curated_at_sk_cross_border_gewerbe_service_connector_pack(
  p_payload jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if p_payload->>'schemaVersion' <> '1'
     or p_payload->>'packId' is distinct from 'at_sk_cross_border_gewerbe_service'
  ...
  if p_payload->>'topicFamily' is distinct from 'TRADE_SERVICE_AUTHORIZATION' then
  ...
  if jsonb_typeof(p_payload->'euClaimRefs') is distinct from 'array' then
    raise exception 'CONNECTOR_INVALID_EU_REFS';
  end if;
  ...
  v_actor := knowledge_factory_internal.stable_knowledge_factory_id(
    ...
    'AT:SK:cross-border-gewerbe-service-authorization'
  );
  ...
  for v_process_row in select value from jsonb_array_elements(v_corridor) loop
    ...
    insert into public.knowledge_processes(
      ...
      v_corridor_process, 'at_sk_cross_border_gewerbe_service_connector',
...
revoke all on function public.knowledge_ingest_curated_at_cross_border_gewerbe_service_routing_pack(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function public.knowledge_ingest_curated_at_sk_cross_border_gewerbe_service_connector_pack(jsonb)
  from public, anon, authenticated, service_role;
...
comment on function public.knowledge_ingest_curated_at_cross_border_gewerbe_service_routing_pack(jsonb) is
  'AT-SK-0H server-only Austrian cross-border Gewerbe/service routing ingest. Revoke-by-default. Not deployed.';
comment on function public.knowledge_ingest_curated_at_sk_cross_border_gewerbe_service_connector_pack(jsonb) is
  'AT-SK-0H server-only AT-SK cross-border Gewerbe/service connector ingest. Prepared only. Revoke-by-default. Not deployed.';
