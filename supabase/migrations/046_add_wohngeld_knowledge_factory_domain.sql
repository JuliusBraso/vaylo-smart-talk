-- Minimum Knowledge Factory taxonomy extension for Wohngeld
-- federal lifecycle. Additive only: extends the process-group check
-- and the saved 041 domain whitelist. Does not change runtime grants,
-- retrieval, or production data.
...
begin
  ...
  if v_definition not like '%wohngeld%' then
    v_definition := replace(
      v_definition,
      $lit$'einkommensteuer_steuererklaerung'$lit$,
      $lit$'einkommensteuer_steuererklaerung','wohngeld'$lit$
    );
